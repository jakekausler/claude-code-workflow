...
2. Filter to only **unmerged** parents:
   - Read each parent's stage YAML to check if status is "Complete" (preferred). If the parent YAML is unavailable, fall back to checking the parent's PR merge status via `gh pr view` / `glab mr view`.
